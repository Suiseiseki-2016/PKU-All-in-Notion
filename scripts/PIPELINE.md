...
| 云端中转（M2 骨架） | 私有仓 `Suiseiseki-2016/PKU-All-in-Notion-server`（`server/accounts.py` + `relay.py`；服务端件不在本仓库） | 平台账号库（SQLite 单文件：账号/会话/兑换码/用量流水）+ `POST /v1/transcribe`（会话鉴权 → 余额闸 402 → ASR 注入位 → 实际秒数计量扣减 → 音频不落盘）+ `GET /v1/quota` + `POST /v1/redeem`（兑换码充值）+ `POST /v1/recharge`（501 支付预留）+ `/healthz`；上游 ASR 未配置时 503 响亮拒绝；用户不持有任何 key（§1.7）；部署目标 `api.aeoluswu.info`（单一客户端域名；server-a/b/c 为主备运维机） |
